...
The Gay New York City in the second half of the twentieth century was a stubborn response to the risks of Normative New York - the heteronormative cultural topography and sexual regime established during and after the Second World War. In this short article, I use methodologies of data exploratory analysis to examine the available data from the <i>Mapping the Gay Guides</i> project and visualize how the suppressed gay landscape in New York City thrived as a result of, among other factors, non-conforming spatial practices and appropriation of public spaces of the normative city by gay men. In the public sphere, particular locales became associated with gay culture and resistance to <i>status quo</i> by being categorized as dangerous and risky for gay men. 
...
<h3> Appropriating Risky New York <h3/>
...
<h4> Step 1: Formulate the question </h4>

Tha Gay Guides data I worked with was originally divided in two large data sets. The 1965-1989 data set has been uploaded to the project's GitHub repository and is constantly updated and made publicly available for reproducibility.[^9] The 1990-1996 data is not publicly availabe yet. Rather, it has been managed in AirTable bases to which locations from Damron's Guides are transcribed and later geolocated. 
...
<h3> Step 4: Try easy answers first <h3/>
...
```
Now, where were these locations and how were they spread over the city landscape? For this question, let's use three years as data samples: 1970, 1980, and 1990.

```{r}
gaynewyork <- gaynewyork %>% 
  mutate(color = ifelse(gaynewyork$categories == "Accommodations", "orange",
                        ifelse(gaynewyork$categories == "Bars", "red",
                               ifelse(gaynewyork$categories == "Bath Houses", "green",
                                      ifelse(gaynewyork$categories == "Businesses", "blue",
                                             ifelse(gaynewyork$categories == "Cruising Areas", "brown",
                                                    ifelse(gaynewyork$categories == "Gyms", "purple",
                                                           ifelse(gaynewyork$categories == "Organizations", "yellow",
                                                                  ifelse(gaynewyork$categories == "Religious Institutions", "darkcyan",
                                                                         ifelse(gaynewyork$categories == "Restaurants", "royalblue", "grey"))))))))))

pal <- colorFactor(palette = "RdGy", domain = nyc.1970$categories)
...
ggplot() +
  ...
  geom_point(data = nyc.1970, aes(x=lon, y=lat), size = 3, shape = 23, fill = ~catCol(nyc.1970$categories))

  leaflet(data = nyc.1970) %>%
    addTiles() %>%
    addCircleMarkers(~lon, ~lat, color = pal(nyc.1970$categories), popup = paste("Location Name:", nyc.1970$title, ", ", "Type:", nyc.1970$categories)) %>% 
    addLegend(position = "bottomleft", values = ~categories, pal = pal)
```


If the user filtered the map to show only locations that existed in the state of New York in 1980, for example, this is how it would look like:

```{r}
gayguides.complete <- read.csv("~/Risky-New-York/gay guides data/gayguides.complete.csv")

gg.nyc.1980 <- gayguides.complete %>% 
  filter(state == "NY" & Year == 1980)

leaflet(data=gg.nyc.1980) %>% addTiles() %>% addMarkers(~lon, ~lat, popup = paste("Location Name:", gg.nyc.1980$title, ", ", "Type:", gg.nyc.1980$type))
```
...
[^9] Mapping the Gay Guides, Amanda Regan and Eric Gonzaba, (2019-): http://www.mappingthegayguides.org.
[^10]: Roger D. Peng, <i>Exploratory Data Analysis with R<i/> (Lulu, 2016): https://bookdown.org/rdpeng/exdata/